...
public class Haupt1 extends AppCompatActivity {
    ...
    public class Tesseract extends AsyncTask<String, Boolean, Void> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            progressDialog = new ProgressDialog(Haupt1.this);
        ...
        private void doOCR() {
            try {
                ...
                for (Bitmap bitmap : bits) {
                    ...
                    String DATA_PATH = Startbildschirm.instance.getTessDataDirectory();
                    ...
                    /*vocList.removeAll(vocList.stream()
                                    .filter(x -> !x.matches("[Ww][Oo][Rr][Dd][Bb][Aa][Nn][Kk]"))
                ...
                runOnUiThread(Haupt1.this::recreate);
            } catch (Exception ignored){}

            //ToDo: detect Words below
            /*final ResultIterator iterator = tessBaseAPI.getResultIterator();
        final ResultIterator iterator2 = tessBaseAPI.getResultIterator();
        final ResultIterator iterator3 = tessBaseAPI.getResultIterator();
        iterator.begin();
        iterator2.begin();
        iterator3.begin();
    ...
    ArrayList<RectF> rect2nds;
    ArrayList<int[]> dimList = new ArrayList<>();
    ArrayList<int[]> dimListFor2nds = new ArrayList<>();
    ...
    private final Logger log = Logger.getLogger(Haupt1.class.getName());
    ...
    protected void onCreate(Bundle savedInstaceState) {
        ...
        loeschen.setOnClickListener(v -> new LiveSmashBar.Builder(Haupt1.this)
        ...
        speichern1.setOnClickListener(view -> {
            try {
                ...
                ArrayList<Bitmap> bitmapList = new ArrayList<>(Arrays.asList(splitImage(bitmap, detect_words(bitmap), rows)));
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == IMG_CROP_CODE) {
            ...
            if (resultCode == RESULT_OK) {
                ...
                rectFS = detect_words(bitmap).get(0);
                rect2nds = detect_words(bitmap).get(1);
                ...
                bildVorschau.setOnTouchListener((view, motionEvent) -> {
                    if (motionEvent.getAction() == MotionEvent.ACTION_DOWN) {
                        ...
                        //int color = pal.getDominantColor(Color.GRAY);
    ...
    public ArrayList<ArrayList<RectF>> detect_words(Bitmap bitmap) {

        Logger log = Logger.getLogger(Haupt1.class.getName());
        TinyDB tb = new TinyDB(getApplicationContext());
    ...
    public Bitmap prepareEditedBitmap(Bitmap bitmap) {
        ...
        for (int i = 0; i < rectFS.size(); i++) {
            Logger log1 = Logger.getLogger(test.class.getName());
            log1.warning("rectangle : " + rectFS.get(i).toShortString());
            RectEdit rectEdit = new RectEdit(getApplicationContext(),
                    canvas, dimList, recPaint, pos);
            rectEdit.add(color);
        }
    ...
    public void intentForCropResult(Uri uri, int crop_code) {
        ...
        Haupt1.this.startActivityForResult(intent, crop_code);
    }
